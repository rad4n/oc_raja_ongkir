...
class ControllerModuleJournal2SuperFilter extends Controller {
    ...
    public function index($setting) {
        ...
        if ($this->journal2->settings->get('responsive_design')) {
            ...
            if ($device === 'phone' && !Journal2Utils::getProperty($this->module_data, 'module_data.enable_on_mobile')) {
                return;
            }
    ...
    protected function getFilters($module_id, $ajax = FALSE) {
        ...
        if (Journal2Utils::getDevice() === 'desktop' && $this->journal2->settings->get('filter_collapsed_state', 'expanded') === 'collapsed') {
            $this->data['is_collapsed'] = true;
        }
        if (Journal2Utils::getDevice() !== 'desktop' && $this->journal2->settings->get('filter_collapsed_state_mobile', 'expanded') === 'collapsed') {
            $this->data['is_collapsed'] = true;
        }
